...
<nav class="fixed top-0 z-50 w-full bg-white border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700">
    ...
    <div class="px-4 py-3">
        ...
        @auth
            {{-- Periksa apakah pengguna yang login adalah 'siswa' dan memiliki data siswa --}}
            @if(auth()->user()->role === 'siswa' && auth()->user()->siswa)
                <span class="block text-sm text-gray-900 dark:text-white">
                    {{ auth()->user()->siswa->nama_siswa }}
                </span>
            @else
                {{-- Pilihan cadangan untuk peran lain atau jika data siswa tidak terhubung --}}
                <span class="block text-sm text-gray-900 dark:text-white">
                    {{ auth()->user()->username }} {{-- Atau nama tampilan lain yang sesuai untuk non-siswa --}}
                </span>
            @endif
        @else
            {{-- Jika tidak ada pengguna yang login (meskipun dropdown ini biasanya muncul saat sudah login) --}}
            <span class="block text-sm text-gray-900 dark:text-white">Tamu</span>
        @endauth
    </div>
